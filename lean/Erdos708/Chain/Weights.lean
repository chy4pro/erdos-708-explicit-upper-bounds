...
lemma capped_to_weight (z : Weight) (m x : ℕ)
    (hc : hingeSum (capped z) 65 0 m ≤ hingeSum (capped z) 1 x m) :
    hingeSum (w z) 65 0 m ≤ hingeSum (w z) 1 x m := by
  have hL : hingeSum (w z) 65 0 m ≤ (∑ k ∈ Icc 1 m, excess z k) + hingeSum (capped z) 65 0 m := by
    have h := sum_le_sum (s := Icc 1 m) (fun k _ => peel_left z k 65)
...
lemma atoms_to_capped (z : Weight) (m x : ℕ)
    (ha : hingeSum (S (ofWeight z m)) 65 0 m ≤ hingeSum (S (ofWeight z m)) 1 x m) :
    hingeSum (capped z) 65 0 m ≤ hingeSum (capped z) 1 x m := by
  have hL : hingeSum (S (ofWeight z m)) 65 0 m = hingeSum (capped z) 65 0 m := by
    apply sum_congr rfl
    intro k hk
