...
lemma carrier_floor_ratio (A : AtomSystem) (m : ℕ) (hm : 2^48 ≤ m)
    (P : ℕ) (hP : P ∈ carriers A m) : (m : ℝ) / P ≤ gamma * (nu m P : ℝ) := by
  have hp := carrier_cube A m (by omega) P hP
  have hscale : 65536 * P ≤ m := by
    have hc : (65536 * P)^3 ≤ m^3 := by
      calc
        _ = 2^48 * P^3 := by ring
        _ ≤ 2^48 * m^2 := Nat.mul_le_mul_left _ hp
        _ ≤ m * m^2 := Nat.mul_le_mul_right _ hm
        _ = _ := by ring
    by_contra hh
    have hlt : m^3 < (65536 * P)^3 := pow_lt_pow_left₀ (by omega) (by omega) (by omega)
    omega
  have hh := floor_ratio_lower m P (carrier_pos A m P hP) hscale
  change (m:ℝ)/P ≤ gamma * (m/P : ℕ)
  norm_num only [gamma] at ⊢
  linarith
...
lemma F_expanded (A : AtomSystem) (m n : ℕ) :
    F A m n =
      (∑ P ∈ carriers A m, if P ∣ n then 24 * coefficient A m P else 0) -
...
lemma F_window_eq (A : AtomSystem) (m x : ℕ) :
    (∑ n ∈ Icc (x + 1) (x + m), F A m n) =
      ∑ P ∈ carriers A m, (24 * coefficient A m P * (windowCount x m P : ℝ) -
        (24 / K) * coefficient A m P *
          ∑ a ∈ (Rounded.retained (small A m) m).filter (fun a => ¬ a.1 ∣ P),
            delta A m (theta A m P) a * (windowCount x m (P * Rounded.modulus a) : ℝ)) := by
  classical
  have hf (n : ℕ) : F A m n = ∑ P ∈ carriers A m,
      ((if P ∣ n then 24 * coefficient A m P else 0) -
        ((24 / K) * coefficient A m P) *
          ∑ a ∈ (Rounded.retained (small A m) m).filter (fun a => ¬ a.1 ∣ P),
            if P * Rounded.modulus a ∣ n then delta A m (theta A m P) a else 0) := by
    rw [sum_sub_distrib]
...
lemma negative_support_le (A : AtomSystem) (m : ℕ) (hm : 1 ≤ m)
  ...
  have hz (P : ℕ) (hP : P ∈ carriers A m) :
    ...
    apply if_neg
    intro he
    have hh := negative_modulus_le A m hm P hP a (mem_filter.mp ha).1
    omega
  have hpos : 0 ≤ ∑ P ∈ carriers A m, if P = D then 24 * coefficient A m P else 0 := by
    apply sum_nonneg
    intro P hP
    split_ifs
