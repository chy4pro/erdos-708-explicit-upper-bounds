...
gives an explicit certificate for 0/1 weights valid up to about $10^{40}$, and proves the 0/1 case for all $m$ with the
threshold $2$ replaced by $20\ln\ln m$ (Theorem~\ref{thm:A}). Section~\ref{sec:fractional} extends this to arbitrary weights
...
targets $W_\omega=59,\,124,\,321$ defined below)
as well as for various subsets of the primes at $m=400$. The optimal certificates are small: $c_{p^j}=1$ for prime powers with $j\ge2$,
...
so this single certificate proves the 0/1 case of \eqref{eq:TH} for every $m\le10^7$ (checked exactly for each $m$) and fails
for all sufficiently large $m$ (the leading-order crossing is at $\ln\ln m=\tfrac{29}6-M\approx4.57$, i.e.\ $m\approx10^{42}$; in
between the inequality is expected but has not been verified); the linear programme shows that better certificates exist ($907$ against $873$ at
$m=1000$), and whether they exist for every $m$ is open. The number of anchors must depend on $P$: for $P$ the primes in $[53,653]$ and
